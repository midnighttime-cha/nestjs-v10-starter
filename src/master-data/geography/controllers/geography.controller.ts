...
const moduleName = "ภาค";

@ApiTags("Master Data: Geography (ภาค)")
@Controller('master-data/geography')
export class GeographyController {
    constructor(
        private geographyServices: GeographyService,
    ...
    async findData(@Res() res, @Req() req, @Query() query) {
        ...
        const resdata = await this.geographyServices.findData(query, pages);
    ...
    async findByIdData(@Res() res, @Req() req, @Param() params) {
        const resdata = await this.geographyServices.findOneData(null, params.id);
    ...
    async create(@Req() req, @Res() res, @Body() body: GeographyCreateDTO, @User() payload) {
        const responseData = await this.geographyServices.create(body, payload.id);

        await this.resdata.addEventLog(payload.id, req, responseData, moduleName, {
            userId: payload.id,
            moduleId: responseData.id,
            module: "MASTERDATA_GEOGRAPHY",
    ...
    @UseGuards(new AuthGaurd())
    @UsePipes(new MyValidationPipe)
    async update(@Res() res, @Body() body: GeographyUpdateDTO, @User() payload, @Req() req, @Param('id') id: number) {
        const responseData = await this.geographyServices.update(id, body, payload.id);
    ...
    async deleteData(@Param('id') id, @Req() req, @Res() res, @User() payload) {
        const deleted = await this.geographyServices.delete(id, payload.id);
    ...
    async restore(@Param('id') id, @Req() req, @Res() res, @User() payload) {
        const restored = await this.geographyServices.restore(id, payload.id);
