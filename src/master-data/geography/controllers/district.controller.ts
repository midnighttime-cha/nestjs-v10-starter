...
const moduleName = "อำเภอ";

@ApiTags("Master Data: District (อำเภอ)")
@Controller('master-data/district')
export class DistrictController {
    constructor(
        private districtServices: DistrictService,
    ...
    async findData(@Res() res, @Req() req, @Query() query) {
        ...
        const resdata = await this.districtServices.findData(query, pages);
    ...
    async findByIdData(@Res() res, @Req() req, @Param() params) {
        const resdata = await this.districtServices.findOneData(null, params.id);
    ...
    async create(@Req() req, @Res() res, @Body() body: DistrictCreateDTO, @User() payload) {
        const responseData = await this.districtServices.create(body, payload.id);

        await this.resdata.addEventLog(payload.id, req, responseData, moduleName, {
            userId: payload.id,
            moduleId: responseData.id,
            module: "MASTERDATA_DISTRICT",
    ...
    @UseGuards(new AuthGaurd())
    @UsePipes(new MyValidationPipe)
    async update(@Res() res, @Body() body: DistrictUpdateDTO, @User() payload, @Req() req, @Param('id') id: number) {
        const responseData = await this.districtServices.update(id, body, payload.id);
    ...
    async deleteData(@Param('id') id, @Req() req, @Res() res, @User() payload) {
        const deleted = await this.districtServices.delete(id, payload.id);
    ...
    async restore(@Param('id') id, @Req() req, @Res() res, @User() payload) {
        const restored = await this.districtServices.restore(id, payload.id);
